...
mod utils {
    macro_rules! declare_function {
        (
            ...
            ABI.get_or_init(|| {
                let mut function = ton_abi::Function {
                    abi_version: $crate::contracts::utils::declare_function!(@abi_version $($abi)?),
                    name: ($name).to_string(),
                    header: $crate::contracts::utils::declare_function!(@header $($($header),+)?),
                    inputs: $inputs,
                    outputs: $outputs,
                    input_id: 0,
                    output_id: 0,
                };
                let id = $crate::contracts::utils::declare_function!(@function_id function $($id)?);
        ...
        (@header) => { Vec::new() };
        (@header $($header:ident),+) => {
            vec![$($crate::contracts::utils::declare_function!(@header_item $header)),+]
        };
        (@header_item pubkey) => {
