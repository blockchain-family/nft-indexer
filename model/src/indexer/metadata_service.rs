...
use storage::actions::{upsert_nft_attributes, upsert_nft_meta, upsert_nft_meta_columns};
...
impl MetadataService {
    pub async fn refresh_metadata(&self, nft_address: &str, collection: &str) -> anyhow::Result<()> {
        let meta = fetch_metadata(
            MsgAddressInt::from_str(nft_address)?,
            &self.consumer,
        )
        .await;
        ...
        let name =meta.get("name").cloned().unwrap_or_default().as_str().unwrap_or_default().to_string();
        let description =meta.get("description").cloned().unwrap_or_default().as_str().unwrap_or_default().to_string();
