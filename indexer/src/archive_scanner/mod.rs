...
impl ArchivesScanner {
    pub async fn new(config: AppConfig, list_path: PathBuf) -> Result<Self> {
        let list = std::fs::read_to_string(list_path)?;

        Ok(Self {
            //handler: Arc::new(BlocksHandler::new(config.model, config.jrpc).await?),
            list,
        })
    }
