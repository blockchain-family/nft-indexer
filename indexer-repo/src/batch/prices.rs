...
use crate::types::NftPriceHistory;

pub async fn save_price_history(pool: &PgPool, data: Vec<NftPriceHistory>) -> Result<()> {
    let sources = data.iter().map(|e| e.source.0.as_str()).collect::<Vec<_>>();
    ...
    let price_tokens = data
        .iter()
        .map(|e| e.price_token.as_ref().map(|t| t.0.as_str()))
        .collect::<Vec<_>>();
    let nft = data
        .iter()
        .map(|e| e.nft.as_ref().map(|t| t.0.as_str()))
        .collect::<Vec<_>>();
