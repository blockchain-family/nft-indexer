...
from nft_collection c
         ...
         left join lateral ( select json_agg(res.json) as list
                             from ( select json_build_object('traitType', na.trait_type, 'traitValues',
                                    ...
                                    where na.collection = c.address or na.collection = ncc.address
         ...
         left join lateral ( select min(na.min_bid * tup.usd_price) as usd
                             from nft_auction na
                                      ...
                                      join nft n on n.address = na.nft and (n.collection = c.address or n.collection = ncc.address) and not n.burned
         ...
         left join lateral ( select min(ds.price * tup.usd_price) as usd
                             from nft_direct_sell ds
                                      ...
                                      join nft n on n.address = ds.nft and (n.collection = c.address or n.collection = ncc.address) and not n.burned
         ...
         left join lateral ( select count(1) as cnt, count(distinct owner) as owners_count
                             ...
                             where n.burned is false
                               and (n.collection = c.address or n.collection = ncc.address) ) nft_counter on true
         left join lateral ( select sum(coalesce(tup.usd_price * nph.price, 0)) as usd
                             ...
                             where nph.collection = c.address or nph.collection = ncc.address) total_volume on true;