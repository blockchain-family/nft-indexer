...
BEGIN
    ...
    if p_full then
        ...
        FROM nft_events ne
                 join nft_collection nc on ne.collection = nc.address and nc.verified
        WHERE ne.local_created_at > (NOW() - INTERVAL '15 minutes')
          and (ne.computed_event_kind is not null or ne.event_type = 'nft_burned');
        set enable_seqscan = on;

        CREATE INDEX idx_temp_address ON address_time_temp(address);
        ...
        BEGIN
            ...
            from nft n
                     ...
                     join address_time_temp att on att.address = n.address
