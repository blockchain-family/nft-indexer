...
alter table nft_collection_details alter column created set not null;

alter table nft_collection_details alter column updated set not null;

CREATE OR REPLACE FUNCTION trigger_insert_collections_details()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO nft_collection_details (address, name, description, verified, owner, wallpaper, logo, first_mint, created, updated, total_count)
    VALUES (NEW.address, NEW.name, NEW.description, NEW.verified, NEW.owner, NEW.wallpaper, NEW.logo, NEW.first_mint, NEW.created, NEW.updated, 0)
