...
fn _get_account_addr(event: &Extracted) -> String {
    event.tx.account_id().to_hex_string()
}

fn _get_created_at(event: &Extracted) -> Result<i64> {
...
fn _get_created_lt(event: &Extracted) -> Result<i64> {
    match event.message.header() {
...
fn _get_token_processor<'a, T>(
    tokens: &'a [Token],
    mapper_fn: impl Fn(&'a TokenValue) -> Option<T> + Clone + 'static,
) -> (impl Fn(&'a str) -> Result<T> + Clone + '_) {
    move |token_name| _get_token_value(tokens, token_name, &mapper_fn)
}

fn _token_to_big_decimal(token: &TokenValue) -> Option<BigDecimal> {
    match token {
...
fn _token_to_str(token: &TokenValue) -> Option<String> {
